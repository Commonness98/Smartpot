...
# 단위 환산 상수. Heat_cons(MJ)와 Elec*(kWh)를 합치기 전 통일하는 데 쓴다.
MJ_PER_KWH = 3.6
...
def load_wur_raw(team: str = WUR_TEAM) -> pd.DataFrame:
    ...
    # 단위 정규화: WUR ReadMe 기준 Heat_cons는 MJ/m²/day, ElecHigh/ElecLow는
    # kWh/m²/day 로 서로 다르다. 이전 구현은 이 둘을 그대로 더해(MJ+kWh) 단위가
    # 성립하지 않는 값을 만들었고, 그 값에 원/kWh 단가를 곱해 비용을 계산했다.
    # 난방분을 kWh로 환산한 뒤 합산해 전체를 kWh/m²/day 로 통일한다.
    heat_mj = pd.to_numeric(res["Heat_cons"], errors="coerce").fillna(0)
    elec_kwh = (pd.to_numeric(res["ElecHigh"], errors="coerce").fillna(0)
                + pd.to_numeric(res["ElecLow"], errors="coerce").fillna(0))
    res["energy_total"] = heat_mj / MJ_PER_KWH + elec_kwh
